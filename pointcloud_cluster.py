# test script to cluster pointclouds
...
for pts_itr in range(1,181,3):
    ...
    labels = np.array(pts_pcd.cluster_dbscan(eps=.30, min_points=5, print_progress=True))
    max_label = labels.max()
    print("Pc has %d clusters" % (max_label + 1))
    colors = cmap(labels / (max_label if max_label > 0 else 1))
    print("color shape: " + str(colors.shape))
    colors[labels < 0] = 0
    pts_pcd.colors = o3d.utility.Vector3dVector(colors[:, :3])
    geom.append(pts_pcd)
