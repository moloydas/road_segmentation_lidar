...
for pts_itr in range(48,181,3):
    ...
    for i in range(0,5):
        hist,_ = np.histogram(augmented_range_image[i,:,5], bins=50,range=(1,40))
    ...
    max_val = np.array(max_freq)[:,1] + 1
    min_val = np.array(max_freq)[:,1] - 1
    for i in range(0,5):
        for j in range(augmented_range_image.shape[1]):
            if augmented_range_image[i, j, 5] < max_val[i] and augmented_range_image[i, j, 5] > min_val[i]:
                drivable_region.append(augmented_range_image[i, j, :3])
                plt.plot(j,augmented_range_image[i, j, 5],'go')
